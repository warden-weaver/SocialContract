...
contract TimeLockPool is BasePool, ITimeLockPool {
    ...
    function deposit(uint256 _amount, uint256 _duration, address _receiver) external virtual override {
        _deposit(_amount, _duration, _receiver);
    }

    function _deposit(uint256 _amount, uint256 _duration, address _receiver) internal {
    ...
    function withdraw(uint256 _depositId, address _receiver) public {
    ...
    // Note: no need to remove multiplier concept as long as maxBonus is 0. It has no different.
...
contract PublicSale is Ownable {
    ...
    modifier whenPublicSaleActive() {
        ...
        require(saleStartTime > 0, "saleStartTime is not set");
        require(block.timestamp >= saleStartTime, "Public sale is not started");
        _;
    }
...
contract TimeLockNonTransferablePool is TimeLockPool, PublicSale, ReentrancyGuard {
    uint256 public constant STAKE_AMOUNT = 3000 ether; // 3000 TOKEN
    ...
    function deposit(uint256, uint256, address) external override pure {
        revert("DIRECT_DEPOSIT_DISABLED");
    }

    // reward not enabled
    function claimRewards(address) external override pure {
        revert("REWARDS_DISABLED");
    }

    
    /// @dev Deduct token and split into 10 splits and deposit with different lock time
    /// The 1st deposit will be locked 1 year, 2nd will be 1 year 3 days, and so on.
    function depositForWhitelist() external whenPublicSaleActive nonReentrant {
        // TODO: More validations?
        require(depositToken.balanceOf(_msgSender()) >= STAKE_AMOUNT, "depositForWhitelist: not enough balance");
        require(getDepositsOfLength(_msgSender()) == 0, "depositForWhitelist: already deposit");
        
        uint256 split = 10; // split deposit to 10 times
        uint256 amountPerSplit = STAKE_AMOUNT / split; // deposit 300 tokens each split
        uint256 vestingPerSplit = 30 days / split; // add 3 days longer for each deposit

        for (uint256 i = 0; i < split; i++) {
            uint256 duration = MIN_LOCK_DURATION + (i * vestingPerSplit);
            _deposit(amountPerSplit, duration, _msgSender());
        }
    }

    /// @dev withdraw all unlocked deposits, will do nothing if no unlocked deposit
    function withdrawAllAvailable() external nonReentrant {
        // double check with deposit balance and length
        uint256 depositLength = getDepositsOfLength(_msgSender());
        require(depositLength > 0, "withdrawAllAvailable: no deposit in list");
        require(getTotalDeposit(_msgSender()) > 0, "withdrawAllAvailable: no deposit");

        // withdraw MUST be made from LAST to first to ensure all deposits are checked.
        // because after each withdraw, last deposit will be move to replaced the withdrawn one. 
        // see more in: TimeLockPool.withdraw
        for (uint256 i = depositLength; i > 0; i--) {
            uint256 depositId = i - 1; // need to minus here to avoid underflow in loop

            // withdraw if unlocked
            if (block.timestamp >= depositsOf[_msgSender()][depositId].end) {
                withdraw(depositId, _msgSender());
            }
        }
    }
}
